...
func (e *CursorExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	translated = helps.ApplyPayloadConfigWithRoot(e.cfg, model, to.String(), "", translated, originalTranslated, requestedModel, "")
...
func cursorAddIfNotEmpty(segments *[]string, value string) {
...
func cursorPromptFromOpenAIPayload(payload []byte) string {
	...
	if len(segments) == 0 {
		...
		// Walk messages/tools/functions via gjson directly; the package-level
		// collect* helpers were removed upstream, so inline a minimal sweep.
		root.Get("messages").ForEach(func(_, msg gjson.Result) bool {
			if t := cursorExtractMessageContent(msg); t != "" {
		...
		cursorAddIfNotEmpty(&fallback, root.Get("input").String())
		cursorAddIfNotEmpty(&fallback, root.Get("prompt").String())
